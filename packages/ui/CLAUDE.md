...
- **focus-visible**: `border-ring` (bordo pieno, non traslucido) + `ring-2 ring-ring/50` come glow supplementare — il ring da solo a opacità 50% è ~1.9:1 e non basta da solo per SC 1.4.11; il bordo pieno con `--ring` è ~4.4:1 su bianco. **Eccezione**: se il bordo a riposo è già `--ring` (es. `outline-brand` di `button.tsx`), al focus il bordo non cambia e resterebbe solo il glow al 50%: lì il ring va pieno (`focus-visible:ring-ring`, ~5:1 su bianco). E per togliere il contorno nativo si usa `outline-hidden`, non `outline-none`: in Tailwind v4 `outline-hidden` lascia un contorno trasparente che in modalità a contrasto elevato (forced colors) diventa visibile, mentre il ring (un box-shadow) lì sparisce.
