...
- `src/components/sections/` — blocchi delle pagine. La home è solo la hero (MODIFICHE del 23 e del 24/09/2026): `HeroSection` con la scritta "Scopri i nostri corsi" (h2 che dà il nome alla lista), i tre bottoni verdi delle aree (`AreaLinkTile`) e "Visita il sito di ECO-TER". "Corsi in calendario" è diventata la pagina `/calendario-corsi` (per ora "date in arrivo": le edizioni andranno lì quando ci saranno le date). La fascia `CtaFinaleSection` è stata tolta solo dalla home: resta su Chi Siamo, FAQ e Formazione su misura. Aree formative, "Perché ECO-TER", "Come funziona", anteprima FAQ e vetrina "Corsi in evidenza" (con `featured-courses.json`) sono state tolte: se servissero di nuovo, sono nella storia git prima del merge di `feat/home-essenziale`.
- `src/components/layout/`, `src/components/catalog/`, `src/components/course/` — layout, catalogo, card corso. Le card di aree, sotto-aree e corsi (`CatalogCard`, `CourseCard`) sono cliccabili per intero: il link è sul titolo e si allarga alla card con `after:absolute after:inset-0`, il focus è disegnato sulla card (`has-[a:focus-visible]`) e in fondo c'è solo un'indicazione decorativa ("Vedi i corsi" / "Vedi il corso") nel colore dell'area (`accentTextClass` in `src/lib/category-ui.ts`). Niente bottoni dentro queste card (richiesta del 24/09/2026).
- Header (`src/components/layout/Header.tsx`): voci da `mainNav` in `src/config/nav.ts` (Calendario corsi · Formazione su misura · Chi Siamo · FAQ · Contatti, uguali nel menu mobile; la pagina "Formazione su misura", prima "Soluzioni Aziendali" e poi "Aziende e professionisti", sta su `/soluzioni/aziende`), niente mega menu. Le voci stanno nell'header da `xl` (1280px) in su, sotto c'è il menu mobile; il campo di ricerca c'è da `lg` in su (il suggerimento "Ctrl K" solo sotto `xl`), sotto resta l'icona. Da `xl` la riga dell'header ha 1120px, ~1100 con la barra di scorrimento classica di Windows, e oggi ne usa ~1060: se aggiungi o allunghi una voce, riprova a 1280px con la barra visibile. Menu mobile e ricerca usano il `Dialog` di `@ecoter/ui` (focus intrappolato, Esc, ritorno del focus): non rifare finestre modali a mano.
