...
- Header (`src/components/layout/Header.tsx`): voci da `mainNav` in `src/config/nav.ts` (Aziende e professionisti · Chi Siamo · FAQ · Contatti, uguali nel menu mobile; la pagina "Aziende e professionisti" sta su `/soluzioni/aziende`), niente mega menu. Menu mobile e ricerca usano il `Dialog` di `@ecoter/ui` (focus intrappolato, Esc, ritorno del focus): non rifare finestre modali a mano.
