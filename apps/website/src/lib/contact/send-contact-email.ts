...
/**
 * Delivery is optional in dev: without `RESEND_API_KEY` + `CONTACT_TO_EMAIL`
 * set, a submission still validates and returns success, it's just logged
 * instead of emailed — the form works end-to-end locally with zero external
 * config (see .env.example). Resend is imported dynamically, only when a
 * key is actually configured, so the package never loads in the common
 * "not configured" path.
 *
